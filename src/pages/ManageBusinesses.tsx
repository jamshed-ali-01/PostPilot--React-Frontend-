...
import { Loader2, Building2, Package, Calendar, AlertCircle, ShieldAlert, CheckCircle2 } from "lucide-react";
    ...
    const toggleMutation = useMutation({
        mutationFn: async ({ id, isActive }: { id: string, isActive: boolean }) => {
            return graphqlRequest(`
                mutation ToggleBusinessStatus($businessId: String!, $isActive: Boolean!) {
                    toggleBusinessSubscription(businessId: $businessId, isActive: $isActive) {
                        id
                        isActive
                    }
                }
        ...
        onSuccess: () => {
            ...
            toast({ title: "Status Updated", description: "The business status has been updated successfully." });
        },
        onError: (err: any) => toast({ title: "Error", description: err.message, variant: "destructive" })
    });

    const handleToggleStatus = (id: string, currentStatus: boolean, businessName: string) => {
            ...
            <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 bg-gradient-to-r from-card to-card/50 rounded-2xl border shadow-sm gap-4">
                    ...
                    <div className="bg-primary/10 text-primary px-4 py-2 rounded-xl font-medium flex items-center gap-2">
                        ...
                        <span>Total Businesses: {data?.businesses?.length || 0}</span>
                    </div>
                </div>
                            ...
                            return (
                                <Card key={business.id} className="relative overflow-hidden flex flex-col hover:border-primary/50 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group bg-card">
                                    <CardHeader className="pb-4 border-b bg-muted/10 relative z-0">
                                        <div className={`absolute top-0 left-0 w-full h-1 transition-colors duration-500 ${business.isActive ? "bg-gradient-to-r from-success to-emerald-400" : "bg-gradient-to-r from-destructive to-rose-400"}`} />

                                        <div className="flex justify-between items-start gap-4 pt-2">
                                            <div>
                                                <CardTitle className="text-xl font-bold tracking-tight">{business.name}</CardTitle>
                                                <div className="flex items-center gap-2 mt-2">
                                                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${business.isActive ? "bg-success/15 text-success-foreground" : "bg-destructive/15 text-destructive"}`}>
                                                        {business.isActive ? <CheckCircle2 className="w-3.5 h-3.5" /> : <ShieldAlert className="w-3.5 h-3.5" />}
                                                        {business.isActive ? "Active" : "Banned / Inactive"}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {/* Admin Controls */}
                                    <div className="p-4 mt-auto border-t bg-muted/20 flex items-center justify-between">
                                        ...
                                        <Switch
                                            ...
                                            onCheckedChange={() => handleToggleStatus(business.id, business.isActive, business.name)}
